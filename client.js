            ...
            switch (op) {
                ...
                case 0: // Dispatch
                    if (eventType === 'READY') {
                        ...
                        // ✅ تعديل السلوك عند الانتقال أو الخروج من الروم
                        else if (
                            d.user_id === this.user_id &&
                            d.guild_id === this.guildId
                        ) {
                            if (d.channel_id === null) {
                                // 🟢 البوت طلع من كل الرومات
                                if (this.autoReconnect.enabled) {
                                    if (this.ignoreReconnect) {
                                        this.emit('debug', 'Already reconnected, ignoring this event');
                                        return;
                                    }
                                    ...
                                    if (this.reconnectAttempts < this.autoReconnect.maxRetries) {
                                        ...
                                        this.emit('debug', `🔁 Bot left all voice channels. Rejoining... (${this.reconnectAttempts}/${this.autoReconnect.maxRetries})`);
                                ...
                                // ⚠️ انتقل لروم آخر داخل نفس السيرفر → لا تفعل شيء
                                this.emit('debug', `⚠️ Bot moved to a different voice channel (${d.channel_id}), no action taken`);
                            }
                        }
                    }
                    break;
